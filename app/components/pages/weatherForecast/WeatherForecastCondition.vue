...
v-app
    form.condform(@submit.prevent="submit")
            ...
            .setting.meteorological-observatory
                v-select(
                    v-model="selectedMeteorologicalObservatory"
                        ...
                        return-object
                        @change="changeMeteorologicalObservatory"
                        required
                )
            .setting.area
                v-select(
                    ...
                    :items="selectedMeteorologicalObservatory != null ? selectedMeteorologicalObservatory.largeAreas : []"
                    :rules="[(v) => !!v || '必須項目です']"
                    label="地域"
                    item-value="largeAreaCode"
                    item-text="largeAreaName"
                    prepend-icon="mdi-map-marker-radius"
                    @change="changeLargeArea"
                    required
                )
            .setting.interval
                .interval-source
                    .interval-source-year
                        v-select(
                            ...
                            :items="selectSourceYears"
                            label="年"
                            required
                            @change="changeIntervalSourceYear"
                        )
                    .interval-source-month
                        v-select(
                            ...
                            :items="selectSourceMonths"
                            label="月"
                            required
                            @change="changeIntervalSourceMonth"
                        )
                    .interval-source-day
                        v-select(
                ...
                .interval-target
                    v-select(
                        ...
                        :items="dicisionTargetDates()"
                        item-text="stringJpType"
                        item-value="dateType"
                        label="月"
                        required
                    )
        ...
        .submit
            .submit-button-area
                v-btn.submit-button(block color="base" @click="submit") 表示
...
export default Vue.extend({
    ...
    methods: {
        ...
        async submit(): Promise<void> {
            ...
            console.log(this.$store.getters['weatherForecastStore/weatherForecast'])
        },
